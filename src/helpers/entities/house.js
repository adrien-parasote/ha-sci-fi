...
export class House {
  ...
  __build(hass){
    return Object.keys(hass.devices).reduce((floors, device_id) => {
        ...
        if(!Object.keys(floors).includes(floor_id)) floors[floor_id] = new Floor(floor_id, hass);
  ...
  __getEntity(device_id, hass){
    ...
    if(Object.keys(SCI_FI_ENTITIES).includes(entity_kind)) return new SCI_FI_ENTITIES[entity_kind](entity.entity_id, entity.state, entity.attributes.icon, entity.attributes.friendly_name);
    return null;
  }

  __getDeviceEntityState(device_id, hass){
    return hass.states[
            Object.values(hass.entities).find((entity) => entity.device_id == device_id
        ).entity_id
    ];
  }
...
class Floor {
    ...
    getEntitiesByKind(entity_kind){
      return Object.values(this.areas).map((area) => { return area.getEntities(entity_kind)})
      .flat();
    }
...
class Area {
    ...
    isActive(entity_kind){
        ...
        return this.entities[entity_kind].reduce((acc, entity) => acc || entity.active , false);
    }
